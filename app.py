...
MCP_PID_FILE = Path(__file__).parent / ".mcp_pid"
...
def mcp_start():
    ...
    try:
        ...
        for _ in range(30):
            time.sleep(0.1)
            if _check_mcp_running():
                return jsonify({"code": 0, "message": "MCP Server 启动成功", "data": {"pid": proc.pid, "url": MCP_URL}})

        # 端口未监听，检查进程是否还活着
        if proc.poll() is None:
            return jsonify({"code": 0, "message": "MCP Server 进程已启动（端口未就绪，请稍候）", "data": {"pid": proc.pid}})
        else:
            return jsonify({"code": 2001, "message": f"MCP Server 启动失败（exit={proc.returncode}），请查看 mcp_server.log", "data": None})
    except Exception as e:
...
def mcp_stop():
    ...
    if not killed and _check_mcp_running():
        try:
            subprocess.run(["lsof", "-ti", f":{MCP_PORT}", "-kill"], check=False, timeout=5)
            killed = True
        except Exception:
            pass
